/**
 * My Shipping Exchanges Screen
 *
 * Lists all Shipping exchange sessions for the current user.
 */
...
type FilterStatus = "all" | "active" | "transit" | "completed";

export const MyShippingExchangesScreen: React.FC = () => {
  const { t } = useTranslation();
  const navigation =
    useNavigation<ProfileScreenProps<"MyShippingExchanges">["navigation"]>();
  ...
  // Filter Shipping exchanges
  const shippingExchanges = sessions.filter((s) => s.type === "SHIPPING");

  // Apply status filter
  const filteredExchanges = shippingExchanges.filter((session) => {
    const isUserSession =
    ...
    switch (filter) {
      ...
      case "transit":
        return (
          ...
          session.status === "DELIVERED_PENDING_CONFIRMATION"
        );
      case "completed":
  ...
  const filters: { key: FilterStatus; label: string }[] = [
    ...
    { key: "transit", label: t("exchange.status.in_transit") },
