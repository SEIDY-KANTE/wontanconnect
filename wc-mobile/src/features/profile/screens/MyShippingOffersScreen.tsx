...
import { ShippingOfferCard } from "@/features/shipping/components/ShippingOfferCard";
...
export const MyShippingOffersScreen: React.FC = () => {
  const { t } = useTranslation();
  const navigation =
    useNavigation<ProfileScreenProps<"MyShippingOffers">["navigation"]>();
  ...
  const [selectedOffer, setSelectedOffer] = useState<ShippingOffer | null>(
    null,
  );
  ...
  const handleCreate = () => {
    ...
    navigation.navigate("ShippingTab", { screen: "ShippingCreate" });
  };

  const handleOfferPress = (offerId: string) => {
    navigation.navigate("ShippingTab", {
      screen: "ShippingDetail",
      params: { offerId },
    });
  };

  const openActions = (offer: ShippingOffer) => {
  ...
  const renderContent = () => {
    ...
    if (myOffers.length === 0) {
      return (
        <EmptyState
          icon="cube-outline"
          title={t("shipping.myOffersEmptyTitle")}
          message={t("shipping.myOffersEmptyMessage")}
